...
def _resolve_credentials_path(credentials_path: str) -> str:
    ...
    if path.is_absolute():
        return str(path)
    # Относительный путь — ищем рядом с этим файлом
...
def get_today_logs(user_id: int, tz: str = "Europe/Moscow") -> list[dict]:
    ...
    today_prefix = datetime.now(timezone(tz)).strftime("%Y-%m-%d")
    ...
    for row in all_rows[1:]:
        ...
        if record.get("user_id") == str(user_id) and record.get("timestamp", "").startswith(today_prefix):
